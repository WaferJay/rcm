...
RUN apt-get update \
    && apt-get install -y --no-install-recommends tini curl rsync openssh-client \
    && apt-get clean \
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD curl -k -fsS "https://127.0.0.1:${RCM_PORT}/healthz" \
        || curl -fsS "http://127.0.0.1:${RCM_PORT}/healthz"
